...
function createBucket(config, bucket_name) {
  ...
  debug('createBucket - Open connection to bucket: %s', bucket_name)
  debug('createBucket - Configuration: %o', config)

  const bucket_config = config.buckets[bucket_name] || {}

  debug('createBucket - bucket config: %o', bucket_config)

  const cluster = new couchbase.Cluster(config.url)
  const bucket  = cluster.openBucket(bucket_name, bucket_config.password)
