...
const wrap = (channel, config, handler, msg) => null === msg ? null :
  handler( bufferToObj(msg.content), msg.properties, msg )

  .tap(() => config.noAck ? null : channel.ack(msg))
...
const listen = R.curry((channel, config, user_handler) => {
  ...
  if ('function' !== typeof user_handler) {
  ...
  const queue_config   = parseQueueConfig(config)
  const _handler       = wrap.bind(null, channel, config, user_handler)

  return channel.prefetch(handler_config.prefetch)

  .then(() => channel.assertQueue(name, queue_config))

  .then(() => channel.consume(name, _handler, handler_config))
