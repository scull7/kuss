...
module.exports = function MemoryStore(data_set) {
  ...
  // _sort :: Sort -> [a] -> [a]
  const _sort = R.curry((sort, list) => {

    if (R.isEmpty(sort)) return list

    const key     = R.compose(R.head, R.keys)(sort)
    const reverse = R.equals('desc', sort[key])

    return R.compose(
      R.ifElse(R.always(reverse), R.reverse, R.identity)
    , R.sortBy(R.prop(key))
    )(list)
